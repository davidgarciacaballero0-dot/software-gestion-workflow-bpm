...
public class DataInitializer implements CommandLineRunner {
    ...
    private PoliticaWorkflow getOrCreatePoliticaFibra(String idOrg, String idVentas, String idIT, String idFin, String idOp) {
    ...
    private PoliticaWorkflow createPoliticaFibra(String idOrg, String idVentas, String idIT, String idFin, String idOp) {
        List<WorkflowNode> nodes = List.of(
            WorkflowNode.builder().id("start").type(NodeType.START).name("Inicio").uiPosition(new UIPosition(50.0, 250.0)).build(),
            WorkflowNode.builder()
                .id("registro")
                .type(NodeType.USER_TASK)
            ...
            WorkflowNode.builder()
                ...
                .formDefinition(List.of(
                    ...
                    new FormFieldDefinition("f_p_monto", "Monto Cobrado ($)", FormFieldType.NUMBER, true, null),
                    new FormFieldDefinition("f_p_concepto", "Concepto (Ej: Instalación Fibra)", FormFieldType.TEXT, true, null)
                )).build(),
            WorkflowNode.builder()
                .id("instalacion")
                .type(NodeType.USER_TASK)
                ...
                .formDefinition(List.of(
                    new FormFieldDefinition("f_i_tecnico", "Técnico Asignado", FormFieldType.TEXT, true, null),
                    new FormFieldDefinition("f_i_mac", "MAC Address Router", FormFieldType.TEXT, true, null)
                )).build(),
            WorkflowNode.builder().id("rechazo").type(NodeType.END).name("Trámite Rechazado (Sin Cobertura)").uiPosition(new UIPosition(600.0, 400.0)).build(),
            WorkflowNode.builder().id("end").type(NodeType.END).name("Servicio Activado").uiPosition(new UIPosition(1000.0, 150.0)).build()
        );
    ...
    private void seedTramitesVacaciones(PoliticaWorkflow pol, List<Usuario> clientes, String idDep) {
        ...
        for (int i = 1; i <= 10; i++) {
            ...
            tramiteRepository.save(TramiteInstancia.builder()
                ...
                .datosAcumuladosFormulario(Map.of("f_dias", rand.nextInt(15) + 1, "f_motivo", "Vacaciones de prueba"))
                .build());
        }
    }
